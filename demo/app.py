...
class HumanDetectionApp:
    def __init__(self):
        ...
        # 앱 시작시 자동으로 카메라 시작 - StreamManager에서 자동 시작하도록 수정함
    ...
    def _register_socketio_handlers(self):
        ...
        @self.socketio.on("subscribe_topic", namespace="/csi")
        def on_subscribe_topic(data):
            try:
                topic = data.get('topic')
                zone_number = data.get('zoneNumber')
                if topic and self.cada_service.mqtt_manager:
                    print(f"[SocketIO] Subscribing to topic: {topic} for Zone {zone_number}")
                    from demo.config.settings import CSI_TOPICS
                    if topic in CSI_TOPICS:
                        self.socketio.emit('topic_subscribed', {
                            'topic': topic,
                ...
                print(f"[SocketIO ERROR] Subscribe topic: {e}")
                self.socketio.emit('topic_subscribed', {
                    'status': 'error',
                    'message': str(e)
                }, namespace='/csi')
                
        @self.socketio.on("unsubscribe_topic", namespace="/csi")
        def on_unsubscribe_topic(data):
            try:
                topic = data.get('topic')
                if topic:
                    print(f"[SocketIO] Unsubscribing from topic: {topic}")
                    self.socketio.emit('topic_unsubscribed', {
                        'topic': topic,
                        'status': 'success'
                    }, namespace='/csi')
            except Exception as e:
                print(f"[SocketIO ERROR] Unsubscribe topic: {e}")
                self.socketio.emit('topic_unsubscribed', {
                    'status': 'error',
                    'message': str(e)
                }, namespace='/csi')
